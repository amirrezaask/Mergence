...
import { ComposerControl, ComposerControlChevron, ComposerControlIcon } from "./ComposerControl.js"
...
function getProviderModelCapabilities(
  models: ReadonlyArray<ServerProviderModel>,
  model: string | null | undefined,
): { optionDescriptors?: ReadonlyArray<ProviderOptionDescriptor> } {
  const match = models.find(candidate => candidate.slug === model)
  return match?.capabilities ?? { optionDescriptors: [] }
}
...
function getSelectedTraits(
  ...
  void provider
  const caps = getProviderModelCapabilities(models, model)
  ...
  return (
    <>
      ...
      {booleanDescriptors.map((descriptor, index) => {
        ...
        return (
          <div key={descriptor.id}>
            {(selectDescriptors.length > 0 || index > 0) ? <MenuDivider /> : null}
            <MenuGroup>
              <div className="px-2 pt-1.5 pb-1 font-medium text-muted-foreground text-xs">
                {descriptor.label}
              </div>
              ...
              >
                <MenuRadioItem value="on" hideIndicator>
                  On
                </MenuRadioItem>
                <MenuRadioItem value="off" hideIndicator>
                  Off
                </MenuRadioItem>
              </MenuRadioGroup>
            </MenuGroup>
