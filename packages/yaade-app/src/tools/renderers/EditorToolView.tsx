...
import {
  MuxLspHost,
  type MuxLspController,
} from "../../mux/MuxLspHost.js";
...
function EditorToolWorkspace(
  props: EditorToolViewProps & { readonly checkoutPath: string },
) {
  ...
  const [lspController, setLspController] =
    useState<MuxLspController | null>(null);
  const [, refreshBuffers] = useReducer((revision: number) => revision + 1, 0);
  const mountedRef = useRef(true);

  const workspaceManager = useMemo(
    () => new WorkspaceManager(platformFileSystem()),
    [],
  );
  ...
  const buffers = useMemo(
    () => editorBufferServiceFor(workspace, props.use.sessionId),
    [props.use.sessionId, workspace],
  );
  ...
  useEffect(
    () => retainEditorBufferService(workspace, props.use.sessionId),
    [props.use.sessionId, workspace],
  );
  ...
  const openFile = useCallback(
    (nextPath: string) => {
      const absolute = absolutePath(props.checkoutPath, nextPath);
      void openUri(pathToFileUri(absolute));
    },
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col" data-yaade-editor-tool>
      {props.toolbar}
      {workspaceReady ? (
        <MuxLspHost
          workspace={workspace}
          ...
          {activeUri ? (
            <>
              <div className="flex h-8 shrink-0 items-center gap-2 border-b border-border px-3">
                <nav className="min-w-0 flex-1 overflow-hidden" aria-label="File path">
                    ...
                    {breadcrumbSegments.map((segment, index) => {
                      ...
                      return (
                        <li key={`${segment}-${index}`} className="flex min-w-0 items-center">
                          ...
                          <span className={current ? "truncate text-foreground" : "truncate text-muted-foreground"}>
                            {segment}
                          </span>
                        </li>
                      );
              ...
              {activeSnapshot?.externalConflict ? (
                ...
                >
                  ...
                  <Button
                    size="xs"
                    variant="ghost"
                    onClick={() => void buffers.keepMine(activeUri)}
                  >
                    Keep mine
                  </Button>
                  <Button
                    size="xs"
                    variant="ghost"
                    onClick={() => void buffers.reloadFromDisk(activeUri)}
                  >
                    Reload
                  </Button>
                </div>
      ...
      {activeUri && lspStatus === "disconnected" ? (
        <Button
          ...
          onClick={() => void lspController?.restart(activeUri)}
        >
