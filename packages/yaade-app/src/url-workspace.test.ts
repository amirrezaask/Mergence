...
describe("url-workspace", () => {
  ...
  it("builds titles and reverse URL paths", () => {
    ...
    assert.equal(
      urlPathForProjectRoot("/Users/me/dev/foo", "/Users/me"),
      "/dev/foo",
    );
    assert.equal(urlPathForProjectRoot("/Users/me", "/Users/me"), "/~");
    assert.equal(
      urlPathForKnownProject("external id"),
      "/_project/external%20id",
    );
    assert.equal(
      knownProjectIdFromPathname("/_project/external%20id"),
      "external id",
    );
  });
  ...
  it("joins project paths", () => {
    assert.equal(
      joinProjectPath("/Users/me/dev", "yaade"),
      "/Users/me/dev/yaade",
    );
  ...
  it("round-trips deep-linked workspaces, checkouts, and processes", () => {
    ...
    assert.deepEqual(projectRouteFromSearch("?view=changes&checkout=wt-key"), {
      view: "changes",
      workspaceId: null,
      checkoutKey: "wt-key",
      processId: null,
    ...
    assert.equal(
      ...
      "/dev/yaade?view=editors&s=ses-1&checkout=wt-key&search=srch-1&q=createRouter&file=src%2Frouter.ts&line=42&column=7&preview=0",
    );
    assert.deepEqual(
      projectRouteFromSearch(
        "?view=editors&s=ses-1&checkout=wt-key&search=srch-1&q=createRouter&file=src%2Frouter.ts&line=42&column=7&preview=0",
      ),
      {
        view: "editors",
        workspaceId: "ses-1",
