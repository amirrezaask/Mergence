...
import type { AgentProvider, PanelId, YaadeTheme } from "@yaade/shared";
import {
  pathToFileUri,
  fileUriToPath,
  canonicalizeFileUri,
...
function uriAtOrBelow(candidate: string, resource: string): boolean {
  return (
    candidate === resource ||
    candidate.startsWith(`${resource.replace(/\/$/, "")}/`)
  );
}
...
export type MuxSurface = "running" | "editors";
...
export function MuxApp({
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("yaade:open-settings", openSettings);
  }, []);
  ...
  const retriedLaunchIdsRef = useRef(new Set<string>());
  const [launchRetryRevision, bumpLaunchRetry] = useReducer(
    (revision: number) => revision + 1,
    0,
  );
  ...
  const workspaceEditTransactionsRef = useRef<
    | import("../editor/workspace-edit-transaction.js").WorkspaceEditTransactionService
    | null
  >(null);
  ...
  const paneBoxesRef = useRef(
    new Map<string, import("./MuxTerminalLayer.js").MuxTerminalSlotBox>(),
  );
            ...
            if (saveAsRequired) {
              ...
              showYaadeToast(
                error instanceof Error
                  ? error.message
                  : `Could not save ${editorLabelFromUri(uri)}`,
                { variant: "destructive" },
              );
            }
            throw error;
  ...
  useEffect(() => {
    ...
    editorOwnerReconciliationRef.current = editorOwnerReconciliationRef.current
      .catch(() => {})
      .then(async () => {
        ...
        const buffers = bufferModule.editorBufferServiceFor(
          workspace,
          sessionId,
        );
        ...
        for (const [key, owner] of desired) {
          try {
            if (!previous.has(key)) {
              const pending = pendingModule.consumePendingInitialContent(
                owner.uri,
              );
              await buffers.open({
                uri: owner.uri,
                ...
                initialDirty:
                  owner.uri.startsWith("untitled:") && pending != null,
              });
            }
  ...
  useEffect(() => {
    ...
    if (
      normalizeAbsPath(editorWorkspacePath) === normalizeAbsPath(sessionCwdPath)
    )
      return;
    void workspace.addFolder(editorWorkspacePath).catch((error) => {
      showYaadeToast(
        error instanceof Error
          ? error.message
          : "Could not open editor checkout",
        { variant: "destructive" },
      );
    });
  ...
  const buildServerPayload = useCallback((): ProjectSessionPayload | null => {
    ...
    return {
      ...
      ...(Object.keys(editorFilesRef.current).length > 0
        ? { editorFiles: { ...editorFilesRef.current } }
        : {}),
      ...(Object.keys(editorViewStates).length > 0 ? { editorViewStates } : {}),
    };
  }, []);
  ...
  const updateWindow = useCallback(
    (windowId: string, mutate: (w: LiveWindow) => LiveWindow) => {
      // Compute from the ref before scheduling React state. Editor launches
      // verify the tree in the same turn, so waiting for a state-updater call
      // would make a successfully placed tab look missing.
      const next = windowsRef.current.map((w) =>
        w.id === windowId ? mutate(w) : w,
      );
  ...
  const allocEditorPane = useCallback(
    (uri: string, line?: number, column?: number): AllocatedEditorPane => {
  ...
  const rememberClosedEditors = useCallback((uris: readonly string[]) => {
    for (const uri of uris) {
      closedEditorUrisRef.current = [
        uri,
        ...closedEditorUrisRef.current.filter(
          (item) => !sameFileTab(item, uri),
        ),
      ].slice(0, 20);
    }
  }, []);
  ...
  const closeWindow = useCallback(
    async (windowId: string, options?: { skipConfirm?: boolean }) => {
      ...
      if (editorBufferIds.length > 0) {
        const { editorBufferServiceFor } =
          await import("../editor/editor-buffer-service.js");
        editorBuffers = editorBufferServiceFor(workspace, sessionIdRef.current);
        if (!(await resolveEditorDirtyClose(editorBufferIds, editorBuffers)))
          return;
      }
  ...
  const closeEditorTab = useCallback(
    async (windowId: string, panelId: PanelId, tabId: string) => {
      ...
      if (
        ...
        showYaadeToast(
          "Save or discard the unsaved buffer before closing it.",
          {
            variant: "warning",
          },
        );
  ...
  const openAgentCliPane = useCallback(
    async (
      ...
      const ptyTabId = terminalTabId(instance.id);
      ...
      trackTerminalPtyId(ptyTabId, instance.ptyId);
      workspace.registerTab({
        id: ptyTabId,
        kind: "terminal",
        label: driver.label,
      ...
      updateWindow(w.id, (live) => placeTerminalPane(live, pane));
      return { tabId: ptyTabId, runId: instance.id };
    },
    [
      canAddTerminalPane,
      ensureProjectWindow,
   ...
   * authoritative; the layout is only a view of them, so an empty payload must
   * not hide a still-running PTY.
   */
  const reconcileLiveTerminalInstances = useCallback(
    (instances: readonly TerminalInstanceInfo[]) => {
      ...
      for (const instance of instances) {
        if (
          instance.workspaceId !== sessionId ||
          !instance.ptyId ||
          (instance.processState !== "running" &&
            instance.processState !== "starting")
        ) {
        ...
        const tabId = terminalTabId(instance.id);
        const existing = terminalSessionForTab(tabId);
        if (existing) {
          if (existing.ptyId !== instance.ptyId) {
            trackTerminalPtyId(tabId, instance.ptyId);
          }
          if (!workspace.tabRegistry.get(tabId)) {
            workspace.registerTab({
              id: tabId,
              kind: "terminal",
              label: existing.agentTitle ?? instance.title,
            });
          }
        } else {
          hydrateTerminalSession({
            tabId,
            cwdRootUri: pathToFileUri(instance.checkoutPath),
            ptyId: instance.ptyId,
            status: "running",
            customLabel: instance.title,
            ...(instance.provider
              ? {
                  agentId: instance.provider,
                  agentTitle: instance.title,
                  agentDriverId: agentDriverIdForMode(instance.provider, "cli"),
                }
              : {}),
          });
          workspace.registerTab({
        ...
        if (
          !listTerminalLeaves(nextWindow.tree).some(
            (leaf) => leaf.ptyTabId === tabId,
          )
        ) {
          nextWindow = placeTerminalPane(
            nextWindow,
            {
              ptyTabId: tabId,
              label: instance.title,
              rootUri: pathToFileUri(instance.checkoutPath),
            },
            "right",
  ...
  useEffect(() => {
    ...
    const reconcile = () => {
      void api
        ...
        .then((instances) => {
          if (cancelled) return;
          reconcileLiveTerminalInstances(
            instances.filter(
              (instance) =>
                instance.workspaceId === sessionId &&
                (instance.processState === "starting" ||
        ...
        .catch(() => {
          /* The project surface remains usable if instance reconciliation is unavailable. */
        });
    };
    reconcile();
    const off = api.onInstanceEvent((event) => {
      if (
        event.instance.projectId === projectId &&
        event.instance.workspaceId === sessionId
      ) {
        reconcile();
      }
    });
  ...
  const openNeovimSplit = useCallback(
    async (
      ...
      updateWindow(windowId, (w) =>
        placeTerminalPane(w, pane, "right", panelId),
      );
    },
    [
      allocTerminalPane,
  ...
  const openEditorInFocused = useCallback(
    (options?: {
      ...
      const focusedEditor = leaves.find(
        (leaf) =>
          leaf.kind === "editor" && leaf.panelId.id === w.focusedPaneId?.id,
      );
      ...
      const panelId =
        editorAnchor?.panelId ?? w.focusedPaneId ?? leaves[0]?.panelId ?? null;
      return openEditorSplit(w.id, panelId, {
        ...options,
        forceNewGroup: options?.forceNewGroup === true,
      });
    },
  ...
  const openInPreferredEditor = useCallback(
    (options?: {
      ...
      if (appearanceSettings.preferredEditor !== "neovim") {
        return openEditorInFocused(options);
      }
      ...
      // Neovim is an intentional alternate editor; the returned path marks
      // the request as accepted even though no Monaco tab is created.
  ...
  const openUntitledFromDrop = useCallback(
    (name: string, content: string) => {
      ...
      void import("@yaade/monaco/pending").then(
  ...
  const tabDnd = useMemo((): TabDndHandlers => {
    return {
      ...
      tabIdsForPanel: (panelId) => {
        ...
        if (!w) return [];
        const view = w.tree.getView(panelId);
        if (view?.kind === "tabs" && isEditorTabId(view.activeTabId)) {
      ...
      onSessionDrop: (sourceWindowId, target, action) => {
        ...
        setWindows((prev) => {
          ...
          const focus = dockSourceLeavesIntoTree(tree, leaves, target, action);
          return prev
  ...
  const cycleEditorBuffer = useCallback(
    (delta: -1 | 1): void => {
      void getActiveDocument().then((current) => {
        ...
        const target =
          buffers[
            (Math.max(index, 0) + delta + buffers.length) % buffers.length
          ];
  ...
  useEffect(() => {
    ...
    void window.yaade?.git
      ...
      .then((branch) => {
        if (!cancelled && branch?.trim())
          setTerminalDefaultBranch(branch.trim());
      })
      .catch(() => undefined);
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    if (!claimMuxLaunchRequest(handledLaunchIdsRef.current, launchRequest.id)) {
      // The first StrictMode effect owns the request. It must be the one that
      // acknowledges it after the side effect has actually completed.
      return;
    }
    ...
    void (async () => {
      ...
      try {
        ...
        if (action.kind === "agent") {
          const driver = AGENT_CLI_DRIVERS.find(
            (item) => item.id === action.driverId,
          );
          if (!driver)
            throw new Error(`Unknown agent provider: ${action.driverId}`);
          ...
          if (!opened) {
            showYaadeToast(
              "Could not open another terminal pane for that agent.",
            );
          }
          ...
          const rootUri = action.checkoutPath
            ...
            : undefined;
          await openTerminalInActiveWindow(
            "right",
          ...
          if (action.filePath) {
            ...
            const openedUri = openInPreferredEditor({
              filePath: action.filePath,
              line: action.line,
              column: action.column,
            });
            if (!openedUri)
              throw new Error("Could not open the requested editor location");
            if (
              appearanceSettings.preferredEditor !== "neovim" &&
              !windowsRef.current.some((window) =>
                Boolean(findPanelWithTab(window.tree, openedUri)),
              )
            ) {
              throw new Error("Editor tab was not installed in the workspace");
            }
        ...
        releaseMuxLaunchRequest(handledLaunchIdsRef.current, launchRequest.id);
        if (!retriedLaunchIdsRef.current.has(launchRequest.id)) {
          retriedLaunchIdsRef.current.add(launchRequest.id);
          bumpLaunchRetry();
        }
        showYaadeToast(
          error instanceof Error
            ? error.message
            : "Could not launch that tool.",
          { variant: "destructive" },
        );
        return;
      }
  ...
  useEffect(() => {
    ...
    const disposers = [
      ...
      commands.register("editor.close", run(closeActiveEditor), {
        id: "editor.close",
        title: "Close Editor",
        category: "File",
        when: hasEditorDocument,
      }),
      commands.register("editor.reopenClosed", run(reopenClosedEditor), {
      ...
      commands.register("editor.save", saveActiveEditor, {
        id: "editor.save",
        title: "Save File",
        category: "File",
      }),
      ...
      commands.register(
        ...
        async () => {
          ...
          const { editorBufferServiceFor } =
            await import("../editor/editor-buffer-service.js");
          if (
            editorBufferServiceFor(workspace, sessionIdRef.current).enableLsp(
              uri,
            )
          ) {
  ...
  useEffect(() => {
    ...
    keymaps.registerUser([
      ...
      bind(
        muxPrefixBindingKey(MUX_PREFIX),
        () => sendPrefixLiteral(),
        noOverlay,
      ),
  ...
  useEffect(() => {
    ...
    window.__yaadeAgent = bridge;
    return () => {
      // Do not clobber a newer project/HQ bridge installed after unmount starts.
      if (window.__yaadeAgent === bridge) delete window.__yaadeAgent;
    };
  }, [
    activeWindow,
    commands,
  ...
  const terminalSurfaceTree = useMemo(() => {
    if (surface !== "running" || !activeWindow) return null;
    return buildTerminalOnlyDisplayTree(activeWindow.tree);
  }, [activeWindow, surface]);
  ...
  const activePtyIds = useMemo(
    () =>
      activeLeaves.filter((l) => l.kind === "terminal").map((l) => l.ptyTabId),
    [activeLeaves],
  );
  ...
  const focusedPtyTabId = useMemo(() => {
    if (surface === "running") {
      if (focusAgentTabId) return focusAgentTabId;
      return (
  ...
  const surfaceEditorBuffers = useMemo((): ModalEditorBuffer[] => {
    ...
    return tabIds.map((tabId) => ({
      ...
      preview: previewEditorTabRef.current === tabId && !editorIsDirty(tabId),
    }));
  }, [activeWindow, editorIsDirty, layoutEpoch, workspace.tabRegistry]);

  // The panel tree is the single source of truth for the visible editor tab.
  // Keeping a second React active-tab state allowed a newly opened result to be
  // registered while the surface continued rendering an older tab.
  const activeEditorPanel = useMemo(() => {
    if (!activeWindow) return null;
    const editors = listPaneLeaves(activeWindow.tree).filter(
      (leaf) => leaf.kind === "editor",
    );
    return (
      editors.find(
        (leaf) => leaf.panelId.id === activeWindow.focusedPaneId?.id,
  ...
  }, [activeWindow, layoutEpoch]);
  const editorsActiveTabId = activeEditorPanel
    ? activeMuxTabInPanel(activeWindow?.tree ?? null, activeEditorPanel.panelId)
    : (surfaceEditorBuffers[0]?.tabId ?? null);

  const runningSidebarItems = useMemo((): InstanceSidebarItem[] => {
    return allPtyIds.map((id) => {
      ...
      return {
        ...
        subtitle: worktree,
        icon: <AgentProviderIcon agent={session?.agentId ?? "terminal"} />,
      };
    });
  ...
  const slotMeasureIds = useMemo(() => {
    ...
    return activeWindow?.zoomedPaneId
      ? isTerminalTabId(activeWindow.zoomedPaneId)
        ...
        : []
      : activePtyIds;
  }, [activePtyIds, activeWindow?.zoomedPaneId, focusedPtyTabId, surface]);
  ...
  const overlayBlocksTerminalFocus =
    paletteOpen || terminalListOpen || settingsOpen || cdOpen || quickOpenOpen;
        ...
        <Suspense fallback={null}>
          <TerminalPanel
            ...
            onPtyId={(tabId, ptyId) => {
              ...
              const provider = agentSession?.agentId as
                | AgentProvider
                | undefined;
              if (
                ptyId &&
      ...
      return (
        <ContextMenu>
          ...
          <ContextMenuContent data-yaade-mux-terminal-context-menu="">
            ...
            {canZoom ? (
              <ContextMenuItem onSelect={() => zoomPane(live.id, ptyTabId)}>
    ...
    (lifecycle: MuxLspController | null) => {
      ...
      void import("../editor/editor-buffer-service.js").then(
        ({ editorBufferServiceFor }) => {
          editorBufferServiceFor(workspace, sessionIdRef.current).setLspHooks(
            lifecycle
              ? {
                  open: lifecycle.open,
      ...
      try {
        ...
        setWindows((previous) => {
          const next = previous.map((item) => ({
            ...
            zoomedPaneId:
              item.zoomedPaneId === sourceUri ? targetUri : item.zoomedPaneId,
          }));
          windowsRef.current = next;
          return next;
        });
  ...
  const renderEditor = useCallback(
    (tabId: string, panelId: PanelId, focused: boolean): ReactNode => {
      ...
      return (
        <Suspense
          fallback={<Skeleton className="min-h-0 flex-1 rounded-none" />}
        >
          <MuxEditorPane
            uri={file.uri}
            line={file.line}
        ...
        prepareOpenResourceReconciliation: async (operations) => {
          ...
          return {
            apply: async () => {
              ...
              await reopen(
                affected,
                (sourceUri) => mapping.get(sourceUri) ?? null,
              );
              ...
              for (const { snapshot } of affected) {
                ...
                if (!newUri) continue;
                nextFiles[newUri] = {
                  ...(originalEditorFiles[snapshot.uri] ?? {
                    uri: snapshot.uri,
                  }),
                  uri: newUri,
                };
  ...
  const onQuickOpenSearch = useCallback(
    async (query: string, signal: AbortSignal): Promise<string[]> => {
      ...
      try {
        return (await search.fileSearch(root, query, { pageSize: 50 }, signal))
          .items;
      } catch (error) {
        ...
        >
          {surface === "running" ? (
            ...
            >
              ...
              <div className="relative min-h-0 min-w-0 flex-1">
                ...
                (terminalSurfacePtyIds.includes(focusedPtyTabId) ||
                  allPtyIds.includes(focusedPtyTabId)) ? (
                  <div
                  ...
                  <div
                    className="grid h-full place-items-center gap-3 px-4 text-center"
                    data-yaade-running-empty=""
                  >
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-2">
                      ...
                      <Button
                        ...
                        variant="secondary"
                        size="sm"
                        data-yaade-running-empty-new-terminal=""
                        onClick={() => {
                          setTerminalCheckoutOpen(true);
                        }}
                      >
            ...
            <div
              ...
              data-yaade-editor-workspace-path={
                editorWorkspacePath ?? sessionCwdPath
              }
            >
                ...
                onEmptyOpenAgent={(driver) => {
                  void openAgentCliPane(driver).catch((error) => {
                    showYaadeToast(
                      error instanceof Error
                        ? error.message
                        : "Could not launch agent",
                      { variant: "destructive" },
                    );
                  });
                }}
                onNewWindow={() => openBrowserProjectTab()}
                gitRootForTab={(tabId) => (gitRoots[tabId] ?? cwdUri()) || null}
        ...
        <Suspense fallback={null}>
          <MuxOverlays
            ...
            onSelectTerminal={(entry) => {
              focusPane(entry.windowId, { id: entry.panelId }, entry.ptyTabId);
            }}
