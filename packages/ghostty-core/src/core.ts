...
export interface GhosttyTheme {
  ...
  /** Complete 256-entry palette for ANSI/256-color cells, when themed. */
...
export class GhosttyTerminalCore {
  ...
  static async create(
    ...
    responsePolicy: GhosttyResponsePolicy = "authoritative",
  ): Promise<GhosttyTerminalCore> {
    const core = new GhosttyTerminalCore(await loadGhosttyRuntime(source));
    try {
      core.initialize(cols, rows, cellWidth, cellHeight, theme, onPtyData, responsePolicy);
      return core;
    } catch (error) {
      core.dispose();
      throw error;
  ...
  /**
   * Ghostty's built-in default cursor is steady, while the previous terminal
   * renderer ran with blinking enabled. Option 23 is the embedder's
  ...
  setTheme(theme: GhosttyTheme): void {
    ...
    if (palette?.length === 256) {
      const palettePointer = this.runtime.alloc(256 * 3);
      const paletteBytes = this.runtime.bytes(palettePointer, 256 * 3);
      for (let index = 0; index < 256; index += 1) {
        const entry = palette[index]!;
        paletteBytes[index * 3] = entry.r;
        paletteBytes[index * 3 + 1] = entry.g;
        paletteBytes[index * 3 + 2] = entry.b;
      }
      this.assertSuccess(
        "ghostty_terminal_set(palette)",
        this.runtime.call("ghostty_terminal_set", this.terminal, 14, palettePointer),
      );
      this.runtime.free(palettePointer, 256 * 3);
    } else {
  ...
  /** Read a DEC/private mode for semantic terminal snapshots. */
  ...
  title(): string {
    this.ensureActive();
    const result = this.runtime.call("ghostty_terminal_get", this.terminal, 12, this.scratch);
    ...
    return decoder.decode(this.runtime.bytes(pointer, length));
  }
