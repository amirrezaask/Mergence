import { GHOSTTY_CELL_WIDE, ghosttyColorsEqual } from "./render-model.js";
import type { GhosttyCell, GhosttyColor, GhosttySnapshot } from "./core.js";
...
function drawUnderline(
  ...
  if (style === 2) {
    context.fillRect(left, y - 2, width, 1);
    context.fillRect(left, y, width, 1);
    return;
  }
  if (style === 3) {
    // A small repeating wave is more legible than flattening curly underlines
    // into a straight rule, while staying stable at fractional cell widths.
    for (let x = 0; x < Math.ceil(width); x += 1) {
      const waveY = y + (x % 4 < 2 ? 0 : 1);
      context.fillRect(left + x, waveY, 1, 1);
    }
    return;
  }
  if (style === 4) {
    for (let x = 0; x < Math.ceil(width); x += 2) {
      context.fillRect(left + x, y, 1, 1);
    }
    return;
  }
  if (style === 5) {
    for (let x = 0; x < Math.ceil(width); x += 5) {
      context.fillRect(left + x, y, Math.min(3, width - x), 1);
    }
    return;
  }
  context.fillRect(left, y, width, 1);
    ...
    for (let column = 0; column < row.cells.length; column += 1) {
      ...
      if (cell.underline || hoveredLink) {
        drawUnderline(
          ...
          metrics.width,
          metrics.height,
          hoveredLink && !cell.underline ? 1 : cell.underline,
        );
      }
