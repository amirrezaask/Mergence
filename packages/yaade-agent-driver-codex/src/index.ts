...
import { AgentCapabilities, AgentConnectionId, AgentDriverDescriptor, AgentTurnId, DriverId, ProviderId, ProviderSessionId, UnsequencedAgentEvent, type AgentActionResponse, type AgentCommandEnvelope, type AgentCommandResult } from "@yaade/agent-protocol"
...
const capabilities = (): AgentThreadConnection["capabilities"] => AgentCapabilities.make({ input: { text: "native", images: "unsupported", workspaceFiles: "native", uploadedFiles: "unsupported" }, threads: { load: "unsupported", resume: "native", fork: "unsupported", list: "unsupported", delete: "unsupported" }, turns: { interrupt: "native", queue: "unsupported", retry: "unsupported", steer: "unsupported" }, output: { reasoning: "unsupported", plans: "unknown", usage: "native", contextWindow: "native", cost: "unknown", subagents: "unknown" }, tools: { streaming: "native", parallel: "unknown", terminal: "unsupported", fileDiffs: "unknown" }, interaction: { permissions: "native", structuredInput: "unsupported", externalUrlInput: "unsupported" }, configuration: { dynamicOptions: "unsupported", slashCommands: "unsupported" } })
...
export class CodexAppServerDriver implements AgentDriver {
  ...
  async openThread(context: AgentDriverContext, request: OpenAgentThreadRequest): Promise<AgentThreadConnection> {
    ...
    try {
      ...
      return new CodexConnection(rpc, queue, client, providerSessionId, context.clock, context.attachments)
...
class CodexConnection implements AgentThreadConnection {
  ...
  constructor(private readonly rpc: JsonlRpc, private readonly queue: AsyncQueue<UnsequencedAgentEvent>, private readonly client: CodexClient, providerSessionId: string | undefined, private readonly clock: AgentDriverContext["clock"], private readonly attachments: AgentDriverContext["attachments"]) { this.binding = { connectionId: Schema.decodeUnknownSync(AgentConnectionId)(`codex:${crypto.randomUUID()}`), ...(providerSessionId ? { providerSessionId: Schema.decodeUnknownSync(ProviderSessionId)(providerSessionId) } : {}) } }
  ...
  async send(envelope: AgentCommandEnvelope): Promise<AgentCommandResult> {
    ...
    try {
      ...
      else if (envelope.command.type === "configuration.set") await this.rpc.request("config/set", { threadId, optionId: envelope.command.optionId, value: envelope.command.value })
