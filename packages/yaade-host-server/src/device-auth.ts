...
export class DeviceAuthService {
  ...
  pair(input: {
    ...
    if (!/^[A-F0-9]{10}$/.test(pairingCode)) throw new Error("invalid pairing code")
    ...
    const row = this.db
      ...
      | undefined
    if (!row || Date.parse(row.expires_at) <= Date.now()) throw new Error("pairing code expired")
    ...
    if (Number(consumed.changes) !== 1) throw new Error("pairing code already used")
  ...
  authenticate(input: {
    ...
    if (!device || device.revoked_at) throw new Error("device is revoked or unknown")
    let valid = false
    try {
