...
function asProvider(value: string | null | undefined): AgentProvider | null {
  ...
  return PROVIDERS.includes(value as AgentProvider) ? (value as AgentProvider) : null
}
...
function asActivityState(value: string | null | undefined): TerminalInstanceActivityState {
  switch (value) {
    case "starting":
    case "working":
    case "running_tool":
...
function asTelemetryState(value: string | null | undefined): TerminalInstanceTelemetryState {
  switch (value) {
    case "connecting":
    case "connected":
    case "degraded":
...
function activityForEvent(event: AgentEvent): TerminalInstanceActivityState | null {
  switch (event.kind) {
...
export class TerminalInstanceService {
  ...
  bindPty(
    ...
    ).run(ptyId, title ?? null, nextTelemetry, timestamp, timestamp, id, generation)
    const bound = Number(changed.changes) === 0 ? this.get(id) : this.updated(id, "instance.updated")
  ...
  markTelemetryDegraded(id: string, generation: number, reason: string): TerminalInstance | null {
    const current = this.get(id)
    if (!current || current.generation !== generation || current.telemetryState === "process_only") {
      return current
    }
    const changed = this.db.prepare(
  ...
  onTelemetry(event: AgentEvent): TerminalInstance | null {
    ...
    if (event.processId && current.ptyId && event.processId !== current.ptyId) return current
    if (event.kind === "process.started" || event.kind === "process.exited") return current
    ...
    return Number(changed.changes) === 0 ? this.get(current.id) : this.updated(current.id, "instance.updated")
  }
  ...
  private scheduleTelemetryGrace(instance: TerminalInstance): void {
    ...
    this.telemetryTimers.set(instance.id, setTimeout(() => {
      ...
      if (Number(changed.changes) > 0) this.updated(instance.id, "instance.updated")
    }, TELEMETRY_GRACE_MS))
  }
  ...
  private ensureProcessColumns(): void {
    ...
    for (const [column, decl] of additions) {
      if (!tableHasColumn(this.db, "terminal_instances", column)) {
        this.db.exec(`ALTER TABLE terminal_instances ADD COLUMN ${column} ${decl}`)
      }
    }
    this.db.exec(`
  ...
  private migrateAgentRuns(): void {
    ...
    if (migrated) return
    const agentTable = this.db.prepare(
      "SELECT name FROM sqlite_master WHERE type='table' AND name='agent_runs'",
    ).get()
    if (!agentTable) {
      this.db.prepare("INSERT OR IGNORE INTO schema_migrations(version) VALUES(14)").run()
      return
    }
    const rows = this.db.prepare(
      `SELECT * FROM agent_runs WHERE removed_at IS NULL`,
    ).all() as Array<{
      run_id: string
