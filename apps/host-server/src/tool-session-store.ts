...
const ToolUseOutputSchema = Schema.Union(ProcessToolOutput, SearchToolOutput);
...
export class ToolSessionStore {
  ...
  private ensureVisibleSession(): void {
    ...
    this.db
      ...
      .run(sessionId(), this.machine, "Session 1", 0, timestamp, timestamp);
  }
  ...
  createSession(title = "New session"): AppSession {
    ...
    return this.getSession(validSessionId(id)) as AppSession;
  }
  ...
  setActiveToolUse(session: SessionId, use: ToolUseId | null): AppSession {
    if (
      ...
      !this.db
        .prepare(
          "SELECT id FROM tool_uses WHERE id=? AND session_id=? AND archived_at IS NULL",
        )
        .get(use, session)
    ) {
      throw new ToolSessionStorageError({
  ...
  createToolUse(record: CreateToolUseRecord): ToolUse {
    ...
    this.db
      .prepare(
        `INSERT INTO tool_uses(id,session_id,kind,title,position,status,project_id,project_path,project_name,
  ...
  reorderToolUses(session: SessionId, ids: readonly ToolUseId[]): ToolUse[] {
    const update = this.db.prepare(
      "UPDATE tool_uses SET position=?,updated_at=? WHERE id=? AND session_id=?",
    );
    const timestamp = now();
    for (const [position, id] of ids.entries())
      update.run(position, timestamp, id, session);
    return this.listToolUses(session);
  }
