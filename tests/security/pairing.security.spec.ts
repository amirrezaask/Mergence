...
async function pairNamedDevice(
  ...
  const pairing = await jsonRequest(origin, "/api/v1/security/pairing-code", {
    method: "POST",
    token: HOST_TOKEN,
  })
  expect(pairing.status).toBe(201)
  const code = String(pairing.body.code ?? "")
  const paired = await jsonRequest(origin, "/api/v1/security/pair", {
    body: {
      code,
      name,
  ...
  const challenge = await jsonRequest(origin, "/api/v1/security/challenge", {
    body: { deviceId },
  })
  expect(challenge.status).toBe(200)
  const nonce = String(challenge.body.nonce ?? "")
  const session = await jsonRequest(origin, "/api/v1/security/session", {
    body: {
      deviceId,
      nonce,
...
async function connectAuthedSocket(origin: string, token: string): Promise<WebSocket> {
  const url = `${origin.replace(/^http/, "ws")}/ws?protocol=2`
...
test.describe("S — device pairing, authentication, and audit", { tag: "@p1" }, () => {
  test("S01 browser device pairing establishes an independent credential", async ({}, testInfo) => {
    await withHarness(testInfo, async harness => {
      ...
      const listed = await jsonRequest(harness.origin, "/api/v1/security/devices", {
        token: HOST_TOKEN,
      })
  ...
  test("S03 revoking a device mid-session closes its live connection", async ({ page }, testInfo) => {
    await withHarness(testInfo, async harness => {
      ...
      const revoke = await jsonRequest(
        harness.origin,
        `/api/v1/security/devices/${device.deviceId}`,
  ...
  test("S06 pairing code is one-time and expires", async ({}, testInfo) => {
    await withHarness(testInfo, async harness => {
      const keys = createDeviceKeys()
      const pairing = await jsonRequest(harness.origin, "/api/v1/security/pairing-code", {
        method: "POST",
        token: HOST_TOKEN,
      })
      const code = String(pairing.body.code ?? "")
      const first = await jsonRequest(harness.origin, "/api/v1/security/pair", {
        body: {
          code,
          name: "First",
          publicKey: keys.publicKey,
      ...
      const reuse = await jsonRequest(harness.origin, "/api/v1/security/pair", {
        body: {
          code,
          name: "Replay",
          publicKey: keys.publicKey,
      ...
      const unused = await jsonRequest(harness.origin, "/api/v1/security/pairing-code", {
        method: "POST",
        token: HOST_TOKEN,
      })
      expireUnusedPairingCodes(harness.dataDir)
      const expired = await jsonRequest(harness.origin, "/api/v1/security/pair", {
        body: {
      ...
      const devices = await jsonRequest(harness.origin, "/api/v1/security/devices", {
        token: HOST_TOKEN,
      })
  ...
  test("S07 replayed challenges and authentication abuse are rejected or rate-limited", async ({}, testInfo) => {
    await withHarness(testInfo, async harness => {
      ...
      const paired = await jsonRequest(harness.origin, "/api/v1/security/pair", {
        body: {
      ...
      const deviceId = String(paired.body.id ?? "")
      const challenge = await jsonRequest(harness.origin, "/api/v1/security/challenge", {
        body: { deviceId },
      })
      const nonce = String(challenge.body.nonce ?? "")
      const signature = keys.signNonce(nonce)
      const first = await jsonRequest(harness.origin, "/api/v1/security/session", {
        body: { deviceId, nonce, signature },
      })
      expect(first.status).toBe(200)
      const replay = await jsonRequest(harness.origin, "/api/v1/security/session", {
        body: { deviceId, nonce, signature },
      })
      expect(replay.status).toBe(401)
      expect(JSON.stringify(replay.body)).not.toContain(String(first.body.token ?? "no-token"))
      const malformed = await jsonRequest(harness.origin, "/api/v1/security/session", {
        body: { deviceId, nonce: "aaaa", signature: "bbbb" },
      })
      expect(malformed.status).toBe(401)
      let limited = false
      for (let i = 0; i < 10; i += 1) {
        const next = await jsonRequest(harness.origin, "/api/v1/security/challenge", {
          body: { deviceId },
        })
        const attempt = await jsonRequest(harness.origin, "/api/v1/security/session", {
          body: {
            deviceId,
  ...
  test("S08 origin and TLS policy is enforced", async ({}, testInfo) => {
    await withHarness(
      testInfo,
      async harness => {
        const denied = await jsonRequest(harness.origin, "/api/v1/system", {
          token: HOST_TOKEN,
          originHeader: "https://attacker.example",
        })
        expect(denied.status).toBe(403)
        expect(rpcErrorCode(denied.body.error)).toBe("ORIGIN_DENIED")
        const approved = await jsonRequest(harness.origin, "/api/v1/system", {
          token: HOST_TOKEN,
          originHeader: "https://yaade.example",
        })
        expect(approved.status).toBe(200)
        const loopback = await jsonRequest(harness.origin, "/api/v1/system", {
          token: HOST_TOKEN,
  ...
  test("S09 credential rotation invalidates the old credential without losing device metadata", async ({}, testInfo) => {
    await withHarness(testInfo, async harness => {
      const device = await pairNamedDevice(harness.origin, "Browser")
      const socket = await connectAuthedSocket(harness.origin, device.token)
      const rotated = await jsonRequest(harness.origin, "/api/v1/security/session/rotate", {
        method: "POST",
        token: device.token,
      })
