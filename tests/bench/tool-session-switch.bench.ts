...
async function openShell(
  page: Awaited<ReturnType<typeof launchWeb>>["page"],
): Promise<void> {
  await page.evaluate(() => {
...
test("session-switch chrome updates within budget", async () => {
  ...
  try {
    ...
    const ids = await page.evaluate(() =>
      (window.__yaadeAgent!.getState().sessions ?? []).map(
    ...
    const result = await runBench({
      ...
      measure: async () => {
        ...
        await page.evaluate(async (id) => {
          ...
          performance.measure(
            "yaade:session-switch",
            "yaade:session-switch:start",
            "yaade:session-switch:end",
          );
        }, ids[0]);
        await page.evaluate(async (id) => {
          performance.clearMarks("yaade:session-switch");
        ...
        const duration = await page.evaluate(() => {
          const measure = performance
            .getEntriesByName("yaade:session-switch")
            .at(-1);
  ...
  try {
    ...
    const result = await runBench({
      ...
      measure: async () => {
        ...
        const duration = await page.evaluate(() => {
          const measure = performance
            .getEntriesByName("yaade:tool-switch")
            .at(-1);
          return measure?.duration ?? Date.now();
        });
        return duration < 10_000 ? duration : Date.now() - started;
